...
namespace PriceTracker.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<PriceTrackerUser> _userStore;
        private readonly IUserEmailStore<PriceTrackerUser> _emailStore;
        ...
        public RegisterModel(
            ...
            IUserStore<PriceTrackerUser> userStore,
        ...
        private PriceTrackerUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<PriceTrackerUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(PriceTrackerUser)}'. " +
                    $"Ensure that '{nameof(PriceTrackerUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<PriceTrackerUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<PriceTrackerUser>)_userStore;
        }
    }
}
