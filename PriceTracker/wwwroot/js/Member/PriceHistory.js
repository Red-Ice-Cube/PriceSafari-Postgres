    ...
    function loadPrices() {
        fetch(`/PriceHistory/GetPrices?storeId=${storeId}&competitorStore=${competitorStore}`)
            ...
            .then(response => {
                ...
                // Pobranie aktualnej wartości z pola wyszukiwania
                const currentSearchTerm = document.getElementById('productSearch').value;

                // Filtrowanie cen na podstawie wartości z pola wyszukiwania i zaznaczonych checkboxów
                let filteredPrices = allPrices;

                // Jeżeli coś jest wpisane w wyszukiwanie, filtruj według nazwy produktu
                ...
                // Filtruj według kategorii, koloru, flag, itp.
                filteredPrices = filterPricesByCategoryAndColorAndFlag(filteredPrices);

                // Renderowanie przefiltrowanych cen
