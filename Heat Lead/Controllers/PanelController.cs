...
namespace Heat_Lead.Controllers
{
    ...
    public class PanelController : Controller
    {
        ...
        public PanelController(UserManager<Heat_LeadUser> userManager, Heat_LeadContext context)
        {
        ...
        public async Task<IActionResult> Index(DateTime? startDate, DateTime? endDate)
        {
            ...
            var latestNews = await _context.News.OrderByDescending(n => n.NewsId).FirstOrDefaultAsync();
            await UpdateOrdersAndWalletForValidation(DateTime.Now);
            ...
            return View("~/Views/Panel/Index.cshtml", model);
        }
        ...
        public async Task<List<CategoryClick>> GetCategoryClicks(string userId, DateTime startDate, DateTime endDate)
        {
            ...
            var categoryClicks = new Dictionary<int, int>();
        ...
        private async Task UpdateOrdersAndWalletForValidation(DateTime currentDate)
        {
            var allRelevantOrders = await _context.Order
                .Where(order => order.ValidationEndDate.HasValue
                           && currentDate > order.ValidationEndDate.Value
                           && order.IsAccepted == true
                           && order.InWallet == false)
                .ToListAsync();

            allRelevantOrders.ForEach(order => order.UpdateValidationStatus());
            _context.Order.UpdateRange(allRelevantOrders);
            await _context.SaveChangesAsync();

            var groupedOrdersByUser = allRelevantOrders
                .Where(order => !order.InValidation)
                .GroupBy(order => order.UserId);

            foreach (var group in groupedOrdersByUser)
            {
                var walletToUpdate = await _context.Wallet.FirstOrDefaultAsync(w => w.UserId == group.Key);
                var validAndAcceptedOrders = group.Where(o => o.IsAccepted).ToList();

                if (validAndAcceptedOrders.Any() && walletToUpdate != null)
                {
                    decimal totalCommission = validAndAcceptedOrders.Sum(o => o.AffiliateCommision);

                    walletToUpdate.TotalEarnings += totalCommission;
                    walletToUpdate.ReadyEarnings += totalCommission;
                    _context.Wallet.Update(walletToUpdate);

                    validAndAcceptedOrders.ForEach(o => o.InWallet = true);
                }
            }
