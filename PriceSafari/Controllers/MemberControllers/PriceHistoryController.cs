...
namespace PriceSafari.Controllers.MemberControllers
{
    ...
    public class PriceHistoryController : Controller
    {
        ...
        //    // Pobieramy ID i datę ostatniego scrapowania
        //    var latestScrap = await _context.ScrapHistories
        //        .Where(sh => sh.StoreId == storeId)
        //        .OrderByDescending(sh => sh.Date)
        //        .Select(sh => new { sh.Id, sh.Date })
        ...
        //    // Pobieramy dane sklepu z możliwymi nazwami
        ...
        //    if (storeData == null)
        //    {
        //        return Json(new { error = "Sklep nie istnieje" });
        //    }

        //    // Tworzymy listę aliasów sklepu
        //    var storeNameVariants = new List<string>();
        //    if (!string.IsNullOrWhiteSpace(storeData.StoreName))
        //        storeNameVariants.Add(storeData.StoreName.ToLower());
        //    if (!string.IsNullOrWhiteSpace(storeData.StoreNameGoogle))
        //        storeNameVariants.Add(storeData.StoreNameGoogle.ToLower());
        //    if (!string.IsNullOrWhiteSpace(storeData.StoreNameCeneo))
        //        storeNameVariants.Add(storeData.StoreNameCeneo.ToLower());

        //    // Użyjemy głównej nazwy do wyświetlenia w UI (np. w polu myStoreName)
        //    string storeNameForLabel = storeData.StoreName ?? "";

        //    // Pobieramy ewentualne wartości z PriceValues
        //    var priceValues = await _context.PriceValues
        //        .Where(pv => pv.StoreId == storeId)
        //        .Select(pv => new { pv.SetPrice1, pv.SetPrice2, pv.PriceStep, pv.UsePriceDiff })
        //        .FirstOrDefaultAsync() ?? new { SetPrice1 = 2.00m, SetPrice2 = 2.00m, PriceStep = 2.00m, UsePriceDiff = true };
        ...
        //    // Filtrowanie, jeśli podano competitorStore
        //    if (!string.IsNullOrEmpty(competitorStore))
        //    {
        //        string competitorStoreLower = competitorStore.ToLower();

        //        pricesQuery = pricesQuery.Where(p =>
        //            p.StoreName != null &&
        //            (
        //                // Czy to nasz sklep (któryś z aliasów)?
        //                storeNameVariants.Contains(p.StoreName.ToLower())
        //                // Albo nazwa konkurencji?
        //                || p.StoreName.ToLower() == competitorStoreLower
        //            )
        //        );
        //    }
        ...
        //    // Pobieramy dodatkowe dane (np. ExternalId) z tabeli Products
        //    var productsWithExternalInfo = await _context.Products
        //        .Where(p => p.StoreId == storeId && p.ExternalId.HasValue)
        //        .Select(p => new { p.ProductId, p.ExternalId, p.ExternalPrice, p.MainUrl, p.MarginPrice })
        //        .ToListAsync();
        ...
        //            // Wszystkie wpisy uznane za "nasz" sklep (dowolny alias)
        //            var myPriceEntries = g.Where(p =>
        //                p.StoreName != null
        //                && storeNameVariants.Contains(p.StoreName.ToLower())
        //            );

        //            // Jeśli mamy wiele wariantów, bierzemy np. najnowszy lub priorytet (kolejność)
        //            var myPriceEntry = myPriceEntries
        //                .OrderByDescending(p => p.IsGoogle == false)
        //                .FirstOrDefault();

        //            // Jeżeli competitorStore jest puste, a nie mamy w ogóle naszej oferty
        //            if (string.IsNullOrEmpty(competitorStore) && myPriceEntry == null)
        //            {
        //                // Jeżeli w parametrach jest source != "all", to pomijamy
        //                if (source.ToLower() != "all")
        //                {
        //                    return null;
        //                }
        ...
        //            bool onlyMe = validPrices.Count > 0 && validPrices.All(p =>
        //                p.StoreName != null
        //                && storeNameVariants.Contains(p.StoreName.ToLower())
        //            );
        ...
        //            // Jeżeli jest competitorStore, znajdź wpis konkurencji
        //            var competitorPriceEntry = validPrices.FirstOrDefault(p =>
        //                !string.IsNullOrEmpty(competitorStore) &&
        //                p.StoreName != null &&
        //                p.StoreName.ToLower() == competitorStore.ToLower()
        ...
        //            // Jeśli brakuje naszej lub konkurencji, to skip
        //            if (!string.IsNullOrEmpty(competitorStore) && (myPriceEntry == null || competitorPriceEntry == null))
        //            {
        //                return null;
        //            }
        ...
        //            // Gdy produkt odrzucony lub cena 0
        //            if (product.IsRejected || bestPrice == 0 || myPrice == 0)
        //            {
        //                isRejectedDueToZeroPrice = true;
        //            }
        ...
        //                if (string.IsNullOrEmpty(competitorStore))
        //                {
        //                    // Logika porównawcza: myPrice vs bestPrice
        //                    if (myPrice == bestPrice)
        //                    {
        //                        if (allBestFromMyStore || bestPriceEntries.Count == 1)
        //                        {
        ...
        //                                    .FirstOrDefault(p => p.Price == secondBestPrice);
        ...
        //                        else
        //                        {
        //                            // Więcej niż jeden sklep z taką samą ceną
        //                            isUniqueBestPrice = false;
        //                            savings = null;
        //                            priceDifference = Math.Round(myPrice.Value - bestPrice.Value, 2);
        //                            percentageDifference = Math.Round(
        ...
        //                    isUniqueBestPrice = myPrice < bestPrice;
        //                    savings = isUniqueBestPrice
        //                        ? Math.Abs(Math.Round(myPrice.Value - bestPrice.Value, 2))
        //                        : (decimal?)null;
        //                    percentageDifference = Math.Round(
        //                        (myPrice.Value - bestPrice.Value) / bestPrice.Value * 100, 2);
        //                    priceDifference = Math.Round(myPrice.Value - bestPrice.Value, 2);
        ...
        //                // Jeżeli wiele sklepów ma tę samą (najlepszą) cenę
        //                var tiedBestPriceEntries = validPrices.Where(p => p.Price == bestPrice).ToList();
        //                if (tiedBestPriceEntries.Count > 1 &&
        //                    tiedBestPriceEntries.Any(e =>
        //                        e.StoreName != null &&
        //                        storeNameVariants.Contains(e.StoreName.ToLower())))
        //                {
        //                    var nonMyBest = tiedBestPriceEntries.FirstOrDefault(x =>
        //                        x.StoreName != null &&
        //                        !storeNameVariants.Contains(x.StoreName.ToLower()));
        //                    if (nonMyBest != null)
        //                    {
        //                        bestPriceEntry = nonMyBest;
        //                    }
        ...
        //                            singleBestCheaperDiff = Math.Round(secondLowestPrice - absoluteLowestPrice.Value, 2);
        //                            decimal diffPercent =
        //                                (secondLowestPrice - absoluteLowestPrice.Value) / secondLowestPrice * 100;
        //                            singleBestCheaperDiffPerc = Math.Round(diffPercent, 2);
        //                        }
        //                    }
        //                }
        ...
        //                    && !validPrices
        //                        .Where(p => p.Price == bestPrice)
        //                        .All(x => x.StoreName != null && storeNameVariants.Contains(x.StoreName.ToLower())),
        //                IsUniqueBestPrice = isUniqueBestPrice,
        //                OnlyMe = onlyMe,
        //                ExternalBestPriceCount = externalBestPriceCount,
        //                IsBidding = bestPriceEntry?.IsBidding,
        ...
        //        myStoreName = storeNameForLabel,
        //        // Lista aliasów (do wykorzystania na froncie)
        //        myStoreNames = storeNameVariants,
        //        prices = allPrices,
        //        missedProductsCount = missedProductsCount,
        //        setPrice1 = priceValues.SetPrice1,
        //        setPrice2 = priceValues.SetPrice2,
        ...
        public async Task<IActionResult> SimulatePriceChange([FromBody] List<SimulationItem> simulationItems)
        {
            ...
            // Funkcja pomocnicza do obliczania rankingu – gdy cena występuje tylko raz, zwraca pojedynczą liczbę,
            // a gdy występuje wielokrotnie – zakres, np. "1-3".
            ...
            foreach (var sim in simulationItems)
            {
                ...
                // wykluczając oferty naszego sklepu.
