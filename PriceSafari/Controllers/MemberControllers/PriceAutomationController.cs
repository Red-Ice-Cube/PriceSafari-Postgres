...
//        private readonly AllegroPriceBridgeService _allegroBridgeService; // 1. Dodajemy serwis

//        public PriceAutomationController(PriceSafariContext context, AllegroPriceBridgeService allegroBridgeService)
//        {
//            _context = context;
//            _allegroBridgeService = allegroBridgeService; // 1. Wstrzykujemy serwis
//        }
//        [HttpGet]
...
//            {
//                // Stara logika dla Ceneo/Google (nadal tylko symulacja/zapis do bazy)
//                await SavePriceComparisonBatch(request, userId, rule);
//                return Ok(new { success = true, count = request.Products.Count });
//            }
//            else if (request.SourceType == AutomationSourceType.Marketplace)
//            {
//                // 2. NOWA LOGIKA: Wywołanie serwisu Allegro (Live Update)
//                var resultCount = await ExecuteAllegroLiveUpdate(request, userId, rule);
...
//        }
//        private async Task<int> ExecuteAllegroLiveUpdate(AutomationExecutionRequest request, string userId, AutomationRule rule)
//        {
//            // A. Pobieramy ID ostatniego scrapingu (wymagane dla logów Batch)
//            var latestScrap = await _context.AllegroScrapeHistories
//                .Where(sh => sh.StoreId == request.StoreId)
//                .OrderByDescending(sh => sh.Date)
//                .Select(sh => sh.Id)
...
//            if (latestScrap == 0) return 0; // Brak historii scrapingu - nie można powiązać zmian

//            // B. Mapujemy dane z Requestu (Frontendu) na DTO Serwisu
//            var itemsToBridge = request.Products.Select(p => new AllegroPriceBridgeItemRequest
//            {
//                ProductId = p.ProductId,         // Wewnętrzne ID
//                OfferId = p.Identifier,          // Allegro Offer ID

//                MarginPrice = p.PurchasePrice,
//                IncludeCommissionInMargin = rule.MarketplaceIncludeCommission,

//                PriceBefore = p.CurrentPrice ?? 0,
//                CommissionBefore = p.CommissionAmount,
//                RankingBefore = p.CurrentRankingAllegro,

//                PriceAfter_Simulated = p.NewPrice, // Nowa cena wyliczona przez automat
//                RankingAfter_Simulated = p.NewRankingAllegro,

//                // Przekazujemy parametry strategii (dla celów analitycznych w logach)
//                Mode = rule.StrategyMode.ToString(),
//                PriceIndexTarget = rule.StrategyMode == AutomationStrategyMode.Profit ? rule.PriceIndexTargetPercent : (decimal?)null,
//                StepPriceApplied = rule.StrategyMode == AutomationStrategyMode.Competitiveness ? rule.PriceStep : (decimal?)null,

//                // Przekazujemy limity, które zostały nałożone
//                MinPriceLimit = p.MinPriceLimit,
//                MaxPriceLimit = p.MaxPriceLimit,
//                WasLimitedByMin = p.WasLimitedByMin,
//                WasLimitedByMax = p.WasLimitedByMax
...
//            // C. Wywołujemy Serwis - to tutaj następuje strzał do API Allegro i zapis do bazy
...
//                // Flagi oznaczające, że to działanie automatu
//                isAutomation: true,
//                automationRuleId: rule.Id
//            );
    ...
    public class PriceAutomationController : Controller
    {
        ...
        public async Task<IActionResult> ExecuteAutomation([FromBody] AutomationTriggerRequest request)
        {
            ...
            else if (rule.SourceType == AutomationSourceType.Marketplace)
            {
                ...
                if (calcResult.ScrapId == 0) return BadRequest("Brak danych historycznych (scrapingu).");
                if (!calcResult.Products.Any()) return Ok(new { success = true, count = 0, message = "Brak produktów." });
