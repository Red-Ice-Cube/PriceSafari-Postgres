...
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore; // <--- 1. TO NAPRAWIA BŁĄD .Include() i .FirstOrDefaultAsync()
using PriceSafari.Attributes;
using PriceSafari.Data;
using PriceSafari.Enums;
using PriceSafari.Models;
using PriceSafari.Models.ViewModels;
using PriceSafari.Services.PriceAutomationService; // <--- 2. TO NAPRAWIA BŁĄD z nazwą serwisu oraz klasami Request (DTO)
using System; // Przydatne do obsługi Exception
using System.Security.Claims;
using System.Threading.Tasks; // Przydatne do Task<>
...
namespace PriceSafari.Controllers.MemberControllers
{
    ...
    public class PriceAutomationController : Controller
    {
        ...
        // Teraz wystarczy wpisać samą nazwę klasy, bo dodaliśmy using namespace'u wyżej
        ...
        public async Task<IActionResult> Details(int id)
        {
            // Dzięki using Microsoft.EntityFrameworkCore te metody teraz zadziałają:
            var rule = await _context.AutomationRules
                .Include(r => r.Store)
                .Include(r => r.CompetitorPreset)
                    .ThenInclude(cp => cp.CompetitorItems)
        ...
        public async Task<IActionResult> ExecuteAutomation([FromBody] AutomationTriggerRequest request)
        {
            // Klasa AutomationTriggerRequest jest teraz widoczna dzięki using PriceSafari.Services.PriceAutomationService;
            if (request == null || request.RuleId <= 0) return BadRequest("Nieprawidłowe żądanie.");
        ...
        public async Task<IActionResult> GetAutomationHistory([FromBody] HistoryRequest request)
        {
            // Klasa HistoryRequest jest teraz widoczna dzięki using PriceSafari.Services.PriceAutomationService;
            if (request == null || request.RuleId <= 0) return BadRequest();
