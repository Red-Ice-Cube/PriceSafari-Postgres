...
//{
//    // === KONFIGURACJA WRGA ===
//    private const bool ENABLE_WRGA_MODE = true;
//    private const decimal WRGA_LOWER_LIMIT = 0.8m;
//    private const decimal WRGA_UPPER_LIMIT = 2.0m;
...
//        // --- NOWOŚĆ: Wyświetlenie tabeli z wynikami OAPV ---
//        PrintOapvTable(allFoundOffers);
...
//                string wrgaUrl = $"https://www.google.com/async/wrga?q={encodedQ}&gl={countryCode}&async=_fmt:prog,gl:{countryCode}";
...
//                    {
//                        if (allFoundOffers.Any(existing => existing.Url == off.Url)) continue;

//                        decimal wrgaPrice = ParsePrice(off.Price);

//                        // Limiter cenowy WRGA
//                        if (baselinePrice > 0 && wrgaPrice > 0)
//                        {
//                            decimal diff = wrgaPrice - baselinePrice;
//                            decimal percentageDiff = diff / baselinePrice;

//                            if (percentageDiff < -WRGA_LOWER_LIMIT || percentageDiff > WRGA_UPPER_LIMIT)
//                            {
//                                Console.WriteLine($"[WRGA SKIP] Odrzucono {off.Seller} ({wrgaPrice}) - cena podejrzana.");
//                                continue;
//                            }
//                        }
...
//                        addedCount++;
//                        Console.WriteLine($"[WRGA OK] Dodano: {off.Seller} ({off.Price})");
//                    }
//                    Console.WriteLine($"[WRGA SUMA] Dodano unikalnych ofert: {addedCount}");
//                }
//                catch (Exception ex)
//                {
...
//        PrintSummaryTable(finalPriceData, countryCode);

//        return finalPriceData;
//    }

//    // === NOWA METODA DO WYŚWIETLANIA TABELI OAPV ===
//    private void PrintOapvTable(List<TempOfferGlobal> offers)
//    {
//        if (!offers.Any()) return;

//        // Sortujemy po cenie (parsowanej)
//        var sorted = offers.OrderBy(o => ParsePrice(o.Price)).ToList();

//        Console.WriteLine("\n");
//        Console.ForegroundColor = ConsoleColor.DarkCyan;
//        Console.WriteLine("==========================================================================================================");
//        Console.WriteLine($" TABELA WYNIKÓW PO OAPV (JSON) - Liczba: {sorted.Count}");
//        Console.WriteLine("==========================================================================================================");
//        Console.ResetColor();

//        Console.WriteLine($"{"Lp.",-4} | {"Sprzedawca",-25} | {"Cena",-10} | {"Dostawa",-10} | {"Razem",-10} | URL");
//        Console.WriteLine("----------------------------------------------------------------------------------------------------------");

//        int idx = 1;
//        foreach (var item in sorted)
//        {
//            decimal p = ParsePrice(item.Price);
//            decimal d = ParseDeliveryPrice(item.Delivery);
//            decimal t = p + d;

//            string seller = Truncate(item.Seller, 25);
//            string url = Truncate(item.Url, 40);

//            // Kolorujemy na szaro, żeby odróżnić od tabeli głównej
//            Console.ForegroundColor = ConsoleColor.Gray;
//            Console.WriteLine($"{idx,-4} | {seller,-25} | {p,-10:F2} | {d,-10:F2} | {t,-10:F2} | {url}");
//            Console.ResetColor();
//            idx++;
//        }
//        Console.WriteLine("----------------------------------------------------------------------------------------------------------");
...
//    // Metoda PrintSummaryTable (dla tabeli końcowej) pozostaje bez zmian jak w poprzednim przykładzie
//    private void PrintSummaryTable(List<PriceData> data, string region)
//    {
//        if (!data.Any())
//        {
...
//        var sorted = data.OrderBy(x => x.PriceWithDelivery).ToList();
...
//        Console.WriteLine($"{"Lp.",-4} | {"Sprzedawca",-25} | {"Cena",-10} | {"Razem",-10} | URL");
//        Console.WriteLine("----------------------------------------------------------------------------------------------------------");
...
//            string seller = Truncate(item.StoreName, 25);
//            string price = item.Price.ToString("F2");
//            string total = item.PriceWithDelivery.ToString("F2");
//            string url = Truncate(item.OfferUrl, 50);

//            Console.WriteLine($"{lp,-4} | {seller,-25} | {price,-10} | {total,-10} | {url}");
//            index++;
//        }
//        Console.WriteLine("----------------------------------------------------------------------------------------------------------\n");
//    }
...
//// Klasa Parser (GoogleShoppingApiParserGlobal) pozostaje bez zmian w stosunku do poprzedniego kodu
//public static class GoogleShoppingApiParserGlobal
//{
//    public static readonly Regex PricePattern = new(
//        @"^(\+)?\s*([\d][\d\s,\.]*[\d])\s+([A-Z]{3}|\p{Sc}|zł|\?)\s*$"
...
//            }
//            if (!string.IsNullOrWhiteSpace(seller) && itemPrice != null && url != null) return new TempOfferGlobal(seller, itemPrice, url, delivery, isInStock);
//            return null;
//        }
//        catch { return null; }
//    }
