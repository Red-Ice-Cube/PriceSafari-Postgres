...
// tryb bez dodatkowego domapowania 
...
// dzialajacy tryb domapowania bez hid
...
//        await Task.Run(() => Parallel.ForEach(products, product =>
//        {
//            // --- LOGIKA KONSOLIDACJI KATALOGÓW ---
//            // Pobieramy wszystkie zadania (główne i dodatkowe) powiązane z tym produktem
//            var relatedCoOfrs = coOfrClasses
//                .Where(co => co.ProductIds.Contains(product.ProductId)
//                             || co.ProductIdsGoogle.Contains(product.ProductId))
//                .ToList();

//            if (!relatedCoOfrs.Any())
//                return;

//            // Logika Extended Info
...
//            // Pobieramy surowe ceny ze wszystkich katalogów
...
//                if (isOurStore && !string.IsNullOrEmpty(coOfrPrice.ExportedName))
//                {
//                    lock (product) { product.ExportedNameCeneo = coOfrPrice.ExportedName; }
//                }
//            }

//            // --- 2. GOOGLE (Z PRIORYTETEM GŁÓWNEGO KATALOGU) ---
//            var googlePricesRaw = allRawPrices
//                .Where(ph => ph.GoogleStoreName != null)
//                .Select(ph => new
//                {
//                    Data = ph,
//                    IsAdditional = relatedCoOfrs.First(co => co.Id == ph.CoOfrClassId).IsAdditionalCatalog
//                })
//                .ToList();

//            // DEDUPLIKACJA SKLEPÓW: Jeśli sklep jest w wielu katalogach, wybieramy GŁÓWNY (IsAdditional = false)
...
//            // --- 3. FALLBACKI ---
//            if (store.UseCeneoXMLFeedPrice && !foundOurStoreCeneo && product.GoogleXMLPrice > 0)
//            {
//                priceHistoriesBag.Add(new PriceHistoryClass
//                {
//                    ProductId = product.ProductId,
...
//                });
//                fallbackUsedList.Add((product.ProductName + " (Ceneo Fallback)", product.GoogleXMLPrice.Value, product.GoogleDeliveryXMLPrice));
//                foundOurStoreCeneo = true;
//            }

//            if (store.UseGoogleXMLFeedPrice && !foundOurStoreGoogle && product.GoogleXMLPrice > 0)
//            {
//                priceHistoriesBag.Add(new PriceHistoryClass
//                {
//                    ProductId = product.ProductId,
...
//                    ShippingCostNum = product.GoogleDeliveryXMLPrice
//                });
//                fallbackUsedList.Add((product.ProductName ?? "(Brak)", product.GoogleXMLPrice.Value, product.GoogleDeliveryXMLPrice));
//                foundOurStoreGoogle = true;
//            }
...
//        // --- ZAPIS (IDENTYCZNY JAK WCZEŚNIEJ) ---
//        scrapHistory.PriceCount = priceHistoriesBag.Count;
//        _context.ScrapHistories.Add(scrapHistory);
//        await _context.SaveChangesAsync();
...
//        for (int i = 0; i < updatedProductsAll.Count; i += CHUNK_SIZE)
//        {
//            var chunk = updatedProductsAll.Skip(i).Take(CHUNK_SIZE);
//            foreach (var prod in chunk)
//            {
//                _context.Products.Attach(prod);
//                _context.Entry(prod).State = EntityState.Modified;
...
public class StoreProcessingService
{
    ...
    public async Task ProcessStoreAsync(int storeId)
    {
        ...
        await Task.Run(() => Parallel.ForEach(products, product =>
        {
            // Pobieramy wszystkie zadania (Main + Additional) dla tego produktu
            var relatedCoOfrs = coOfrClasses
                .Where(co => co.ProductIds.Contains(product.ProductId)
                             || co.ProductIdsGoogle.Contains(product.ProductId))
                .ToList();
            ...
            // --- 2. SEKCJA GOOGLE (KONSOLIDACJA DANYCH) ---
            var googlePricesRaw = allRawPrices
                .Where(ph => ph.GoogleStoreName != null)
                .Select(ph => {
            ...
            // DEDUPLIKACJA: Jeśli ten sam konkurent jest w katalogu głównym i dodatkowym,
            // bierzemy tylko ofertę z GŁÓWNEGO (IsAdditional = false).
            var deduplicatedGoogle = googlePricesRaw
                .GroupBy(x => x.Data.GoogleStoreName.ToLower())
                .Select(g => g.OrderBy(x => x.IsAdditional).First().Data)
                .ToList();
            ...
            foreach (var gp in deduplicatedGoogle)
            {
                ...
                // Sprawdzamy, czy nasz sklep jest w jakimkolwiek powiązanym katalogu
                if (isOurStore) foundOurStoreGoogle = true;

                // Dodajemy cenę do historii (zarówno konkurencji, jak i naszej jeśli jest)
                priceHistoriesBag.Add(new PriceHistoryClass
                {
                    ProductId = product.ProductId,
                    StoreName = isOurStore ? canonicalStoreName : gp.GoogleStoreName,
            ...
            // --- 4. STATUS REJECTED ---
            // Jeśli nasz sklep został znaleziony w KTÓRYMKOLWIEK źródle (Main, CID, HID lub XML), 
            // to IsRejected ustawiamy na false. Ale NIE zmieniamy GoogleUrl w produkcie.
            bool hasStorePrice = foundOurStoreCeneo || foundOurStoreGoogle;
            lock (product)
            {
                if (product.IsRejected == hasStorePrice)
