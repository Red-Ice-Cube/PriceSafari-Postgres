...
namespace PriceSafari.Services.AllegroServices
{
    ...
    public class AllegroApiBotService
    {
        private readonly PriceSafariContext _context;
        private readonly ILogger<AllegroApiBotService> _logger;
        // Używamy JEDNEJ statycznej instancji HttpClient dla wydajności
        private static readonly HttpClient _httpClient = new();
        ...
        // ### ZMIENIONE (Krytyczna optymalizacja) ###
        // Ta metoda przetwarza teraz oferty dla sklepu RÓWNOLEGLE, używając SemaphoreSlim,
        // zamiast sekwencyjnie 'foreach' i zapisywania partiami.
        private async Task<(bool success, int processedCount, string message)> ProcessOffersForSingleStore(StoreClass store)
        {
            _logger.LogInformation("Przetwarzam sklep: {StoreName} (ID: {StoreId})", store.StoreName, store.StoreId);
            ...
            try
            {
                // Ustawiamy limit równoległych zapytań, np. 15, aby nie przekroczyć limitów API
                ...
                foreach (var offer in offersToProcess)
                {
                    // Dodajemy zadanie do listy, ale jeszcze go nie 'await'
                    tasks.Add(Task.Run(async () =>
                    {
                        await semaphore.WaitAsync(); // Czekaj na wolne "okienko"
                        try
                        {
                            // Wywołujemy nową, zoptymalizowaną (równoległą) metodę Fetch
                            var apiData = await FetchApiDataForOffer(accessToken, offer.AllegroOfferId.ToString());
                        ...
                        {
                            // Ten błąd jest krytyczny, rzucamy go dalej
                            throw;
                        }
                        catch (Exception ex)
                        {
                            // Inne błędy logujemy, ale idziemy dalej
                            _logger.LogError(ex, "Błąd podczas przetwarzania oferty {OfferId} dla sklepu {StoreName}", offer.AllegroOfferId, store.StoreName);
                        }
                        finally
                        {
                            offer.IsApiProcessed = true; // Oznacz jako przetworzoną (nawet jeśli był błąd, by nie próbować w kółko)
                            semaphore.Release(); // Zwolnij "okienko"
                        }
                    }));
                }
                ...
                // Zapisujemy wszystkie zmiany do bazy danych JEDEN RAZ na końcu
                await _context.SaveChangesAsync();
            ...
            catch (AllegroAuthException ex)
            {
                ...
                // Oznacz wszystkie jako przetworzone, aby nie próbować ponownie z tym samym złym tokenem
        ...
        // ### ZMIENIONE (Duża optymalizacja) ###
        // Ta metoda pobiera teraz dane RÓWNOLEGLE, tak jak w naszym teście.
        private async Task<AllegroApiSummary?> FetchApiDataForOffer(string accessToken, string offerId)
        {
            try
            {
                // KROK 1: Uruchom niezależne zapytania równolegle
                var offerDataTask = GetOfferData(accessToken, offerId);
                // Używamy nowej, FILTROWANEJ wersji GetAlleDiscountsAsync
                ...
                if (offerDataNode == null) return null; // Błąd pobierania oferty, przerwij

                // KROK 2: Uruchom zapytania zależne od offerDataNode (też równolegle)
        ...
        // ### ZMIENIONE (Duża optymalizacja) ###
        // Metoda w całości zastąpiona naszą nową, FILTRUJĄCĄ wersją N+1.
        private async Task<List<AlleDiscountData>> GetAlleDiscountsAsync(string accessToken, string offerId)
        {
            var activeDiscounts = new List<AlleDiscountData>();
            ...
            // Używamy Task.WhenAll, aby odpytać pasujące kampanie równolegle!
            // To jest dodatkowa mikro-optymalizacja wewnątrz pętli N+1.
            ...
            foreach (var campaign in campaignsArray)
            {
                ...
                {
                    skippedCampaigns++;
                    continue; // Pomiń kampanie inne niż PL
                }
                if (campaignId.Contains("BUSINESS") || campaignId.Contains("B2B"))
                {
                    skippedCampaigns++;
                    continue; // Pomiń kampanie biznesowe
                }
                // ###############################

                // Dodajemy zadanie do listy zamiast je 'await'
                tasks.Add(CheckSingleCampaignAsync(accessToken, offerId, campaignId, campaignName));
            }

            // Czekamy na ukończenie wszystkich zapytań do pasujących kampanii
            var results = await Task.WhenAll(tasks);

            // Zbieramy poprawne wyniki
        ...
        // Wyodrębniona logika sprawdzania pojedynczej kampanii (dla równoległości)
        private async Task<AlleDiscountData?> CheckSingleCampaignAsync(string accessToken, string offerId, string campaignId, string campaignName)
        {
            ...
            if (submittedOffersArray != null)
            {
                foreach (var submittedOffer in submittedOffersArray)
                {
                    ...
                    if (processStatus == "ACTIVE")
                    {
                        // Znaleziono aktywną ofertę, zwracamy dane
                        return new AlleDiscountData(campaignName, submittedOffer?["prices"]);
                    }
                }
            }

            return null; // Brak aktywnej oferty w tej kampanii
        }
