...
namespace PriceSafari.Services.AllegroServices
{
    ...
    public class AllegroApiBotService
    {
            ...
            string accessToken, string offerId)
        {
            try
            {
                // ═══ ZMIANA: Dodano visitsTask do równoległego pobierania ═══
                var offerDataTask = GetOfferDataWithRetry(accessToken, offerId);
                var badgesTask = GetBadgesWithRetry(accessToken, offerId);
                var visitsTask = GetOfferVisitsCountWithRetry(accessToken, offerId);

                await Task.WhenAll(offerDataTask, badgesTask, visitsTask);
                ...
                // ═══ NOWE: Parsowanie Brand (Marka) z tych samych danych oferty ═══
                string? brand = null;
                try
                {
                    var parameters = offerDataNode["productSet"]?[0]?["product"]?["parameters"]?.AsArray();
                    if (parameters != null)
                    {
                        ...
                        if (brandParam != null)
                        {
                            // Najpierw valuesLabels (wartości słownikowe), potem values (wpisane ręcznie)
        ...
        private async Task<int?> GetOfferVisitsCountWithRetry(string accessToken, string offerId)
        {
            ...
            while (attempt < MAX_RETRIES)
            {
                try
                {
                    var apiUrl = $"https://api.allegro.pl/sale/offers?offer.id={offerId}";
                    ...
                    {
                        var visitsStr = offersArray[0]?["stats"]?["visitsCount"]?.ToString();
                        if (int.TryParse(visitsStr, out var visits))
                            return visits;
                    }
