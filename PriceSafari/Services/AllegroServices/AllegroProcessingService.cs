...
//            // (Usunięto pusty SaveChanges, który był w twoim kodzie, nie jest tu potrzebny)
...
//                // Logika aktualizacji EAN (jeśli API go zwróciło)
//                if (sourceOfferToScrape.IsApiProcessed == true && !string.IsNullOrEmpty(sourceOfferToScrape.AllegroEan))
//                {
//                    var newEan = sourceOfferToScrape.AllegroEan;
//                    var productsToUpdateEan = storeProducts
...
//                    // Tworzenie historii cen (dane ze scrapingu)
//                    // Uwaga: tutaj zapisujemy tylko to co widzi scraping (czyli np. 1687.00)
//                    newPriceHistories.Add(new AllegroPriceHistory
//                    {
//                        AllegroProductId = productId,
//                        AllegroScrapeHistory = scrapeHistory,
...
//                    // TUTAJ JEST KLUCZOWA ZMIANA - WERYFIKACJA DANYCH API
//                    // Zapisujemy dane rozszerzone tylko dla NASZEJ oferty (sprawdzamy po SellerName)
//                    // Jeśli scrapedOffer nie jest naszą ofertą, to extended info z API i tak nie ma sensu (bo API pobiera dane dla nas)
...
//                    {
//                        // Przygotuj wartości domyślne prosto z API
...
//                        // --- SANITY CHECK ---
//                        // Sprawdź niespójność: API twierdzi, że jest dopłata (CustomerPrice < SellerRevenue),
//                        // ale scraping twierdzi, że cena widoczna (scrapedOffer.Price) jest inna niż ta z API.

//                        if (finalIsSubsidyActive == true && finalApiAllegroPrice.HasValue)
//                        {
//                            // Jeśli cena ze scrapingu (1687) jest RÓŻNA od ceny z API dla klienta (1499)
//                            // To znaczy, że dopłata nie jest widoczna na froncie.
//                            if (scrapedOffer.Price != finalApiAllegroPrice.Value)
//                            {
//                                // Wykryto anomalię! Wyłączamy flagi promocji w zapisie do bazy.
//                                finalIsSubsidyActive = false;
//                                finalAnyPromoActive = false;

//                                // Opcjonalnie: Ustawiamy ApiAllegroPrice na cenę bazową (Revenue), bo ta realnie obowiązuje
...
//                            AllegroProductId = productId,
//                            ScrapHistory = scrapeHistory,
//                            // Używamy zweryfikowanych zmiennych zamiast surowych z sourceOfferToScrape
...
namespace PriceSafari.Services.AllegroServices
{
    public class AllegroProcessingService
    {
        ...
        public async Task<(int processedUrls, int savedOffers)> ProcessScrapedDataForStoreAsync(int storeId)
        {
            ...
            // Ustawiamy flagę rejected na true dla produktów niespełniających wymagań
            if (allProductIdsToReject.Any())
            {
                // Tutaj używamy ExecuteUpdateAsync, co jest wydajne, ale nie aktualizuje lokalnej listy storeProducts
                // To jest OK, bo poniżej w pętli mamy 'continue' dla tych ID.
                await _context.AllegroProducts
                    .Where(p => allProductIdsToReject.Contains(p.AllegroProductId))
                    .ExecuteUpdateAsync(s => s.SetProperty(p => p.IsRejected, true));
            }
