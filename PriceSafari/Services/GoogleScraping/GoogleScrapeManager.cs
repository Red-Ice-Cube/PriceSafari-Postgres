...
namespace PriceSafari.Services.GoogleScraping
{
    ...
    public static class GoogleScrapeManager
    {
        // === Konfiguracja ===
        public const int BatchSize = 100;
        public const int BatchTimeoutSeconds = 360; 
        public const int ScraperOfflineThresholdSeconds = 120;
        public const int MaxLogEntries = 300;
        ...
        // ZMIANA: Lock jest teraz PUBLICZNY, aby kontroler mógł go używać
        public static readonly object BatchAssignmentLock = new();

        private static int _batchCounter = 0;

        // ZMIANA: Timer do czyszczenia wiszących paczek
        ...
        /// <summary>
        /// Zwraca wszystkie ID tasków które są w aktywnych (nieukończonych) paczkach.
        /// Zwraca TYLKO te, które NIE mają flagi IsTimedOut.
        /// </summary>
        public static HashSet<int> GetAllActiveTaskIds()
        {
            var activeIds = new HashSet<int>();
            ...
            lock (BatchAssignmentLock)
            {
                foreach (var batch in AssignedBatches.Values)
                {
                    // Kluczowe: ignorujemy paczki oznaczone jako TimedOut
                    if (!batch.IsCompleted && !batch.IsTimedOut)
                    {
                        foreach (var taskId in batch.TaskIds)
                        {
        ...
        public static List<(string BatchId, string ScraperName, List<int> TaskIds)> FindAndMarkTimedOutBatches()
        {
            ...
            {
                var batch = kvp.Value;
                // Jeśli paczka nie jest skończona, nie jest oznaczona jako timeout, a jest stara
                if (!batch.IsCompleted && !batch.IsTimedOut && batch.AssignedAt < threshold)
                {
                    batch.IsTimedOut = true;
        ...
        // ===== AUTOMATYCZNE CZYSZCZENIE (TIMER) =====

        private static void OnCleanupTimerTick(object? state)
        {
            try
            {
                if (CurrentStatus != GoogleScrapingProcessStatus.Running) return;

                // 1. Oznaczanie wiszących paczek (URL wracają do puli, bo GetAllActiveTaskIds pomija TimedOut)
                var timedOutBatches = FindAndMarkTimedOutBatches();

                // 2. Oznaczanie martwych scraperów (Offline)
        ...
        public static void ResetForNewProcess()
        {
            ...
            // START TIMERA: Uruchamiaj co 30 sekund
            _cleanupTimer?.Dispose();
            _cleanupTimer = new Timer(OnCleanupTimerTick, null, TimeSpan.FromSeconds(30), TimeSpan.FromSeconds(30));

            AddSystemLog("INFO", "🚀 Rozpoczęto nowy proces scrapowania Google (Timer czyszczący włączony)");
        }
        ...
        public static void FinishProcess()
        {
            ...
            AddSystemLog("SUCCESS", "✅ Proces scrapowania Google zakończony (Timer zatrzymany)");
        }
