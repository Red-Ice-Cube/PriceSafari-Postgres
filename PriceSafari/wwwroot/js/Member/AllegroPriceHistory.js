...
//                                 <p>Obecny narzut: <strong>${oldMarginPercentage.toFixed(2)}%</strong>. Cena zakupu: <strong>${formatPricePL(marginPrice)}</strong>.</p>`
//                            );
//                            return;
//                        }
//                    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ### NOWE ZMIENNE DO SYMULACJI ###
    let currentScrapId = null;
    let selectedPriceChanges = []; // Zarządzane przez AllegroPriceChange.js
    ...
    // ### NOWA FUNKCJA (SKOPIOWANA I ZAADAPTOWANA) ###
    function activateChangeButton(button, actionLine, priceBox, stepPriceApplied, stepUnitApplied) {
        if (!button) {
        ...
        if (!button.dataset.originalText) { // Zapisz oryginalną treść tylko raz
            button.dataset.originalText = button.innerHTML;
        }
        ...
        removeLink.addEventListener('click', function (ev) {
            ...
            // Wyślij event, że zmiana została usunięta
            ...
            // Natychmiast zresetuj ten konkretny box
            ...
            if (item && priceBoxColumnInfo) {
                ...
                if (newSuggestionData) {
                    ...
                    priceBoxColumnInfo.innerHTML = ''; // Wyczyść, jeśli nie ma nowej sugestii
                }
            }
            priceBox.classList.remove('price-changed');
        });
    ...
    function attachPriceChangeListener(actionLine, suggestedPrice, priceBox, productId, productName, currentPriceValue, item) {
        ...
        // Klonowanie, aby usunąć stare listenery
        const newActionLine = actionLine.cloneNode(true);
        actionLine.parentNode.replaceChild(newActionLine, actionLine);
        const newButton = newActionLine.querySelector('.simulate-change-btn');
        ...
        newActionLine.addEventListener('click', function (e) {
            ...
            // Walidacja marży (logika już istniała w twoim pliku)
            if (allegroMarginSettings.useMarginForSimulation) {
                if (item.marginPrice == null) {
                    showGlobalNotification(
                        `<p style="margin:8px 0; font-weight:bold;">Zmiana ceny nie została dodana</p>
            ...
            // Koniec walidacji marży

            const currentSetStepPrice = setStepPrice;
            const currentUsePriceDifference = usePriceDifference;
            const stepUnit = currentUsePriceDifference ? 'PLN' : '%';

            // ### DODANY EVENT DISPATCHER ###
            const priceChangeEvent = new CustomEvent('priceBoxChange', {
                detail: {
                    ...
                    storeId: storeId,
                    scrapId: currentScrapId, // Użyj globalnego scrapId
    ...
    // Funkcja z twojego pliku, bez zmian
    function calculateCurrentSuggestion(item) {
        const currentMyPrice = item.myPrice != null ? parseFloat(item.myPrice) : null;
        const currentLowestPrice = item.lowestPrice != null ? parseFloat(item.lowestPrice) : null;
        const currentSavings = item.savings != null ? parseFloat(item.savings) : null;
    ...
    // Funkcja z twojego pliku, bez zmian (używa 'includeCommissionInMargin' poprawnie)
    function renderSuggestionBlockHTML(item, suggestionData) {
    ...
    function renderPrices(data) {
        ...
        // Wczytaj aktualne zmiany z LS (potrzebne do `existingChange`)
        ...
        if (storedChangesJSON) {
            ...
            } catch (e) { console.error("Błąd parsowania LS w renderPrices", e); }
        }
        ...
        paginatedData.forEach(item => {
            ...
            // ### ZMODYFIKOWANA LOGIKA RENDEROWANIA SUGESTII ###
            const infoCol = box.querySelector('.price-box-column-action');
            infoCol.innerHTML = '';
            ...
            if (suggestionData) {
                let priceToUse = suggestionData.suggestedPrice;
                if (existingChange) {
                    // Nadpisz sugerowaną cenę ceną z LS
                    suggestionData.suggestedPrice = existingChange.newPrice;
                    // Przelicz różnice na podstawie ceny z LS
                    const basePriceForChangeCalc = item.apiAllegroPriceFromUser != null ? parseFloat(item.apiAllegroPriceFromUser) : (item.myPrice != null ? parseFloat(item.myPrice) : 0);
                    suggestionData.totalChangeAmount = existingChange.newPrice - basePriceForChangeCalc;
                    suggestionData.percentageChange = (basePriceForChangeCalc > 0) ? (suggestionData.totalChangeAmount / basePriceForChangeCalc) * 100 : 0;
                    priceToUse = existingChange.newPrice;
                }
                ...
                if (newActionLine) {
                    const myPrice = item.myPrice != null ? parseFloat(item.myPrice) : null;
                    // Zawsze używaj 'priceToUse', która jest albo nową sugestią, albo ceną z LS
                    attachPriceChangeListener(newActionLine, priceToUse, box, item.productId, item.productName, myPrice, item);

                    // Jeśli zmiana pochodzi z LS, aktywuj przycisk od razu
                    if (existingChange) {
                        const button = newActionLine.querySelector('.simulate-change-btn');
                        if (button) {
                            activateChangeButton(button, newActionLine, box, existingChange.stepPriceApplied, existingChange.stepUnitApplied);
                        }
                    }
                }
            } else {
        ...
        if (storedDataJSON) {
            try {
                ...
                if (!storedData.scrapId || storedData.scrapId !== currentScrapId) {
                    ...
                    // Wyślij event, aby poinformować AllegroPriceChange.js o czyszczeniu
    ...
    function refreshPriceBoxStates() {
        ...
        if (currentStoredChangesJSON) {
            try {
                ...
                if (parsedData && parsedData.scrapId === currentScrapId && Array.isArray(parsedData.changes)) {
                    selectedPriceChanges = parsedData.changes; // Synchronizuj lokalną (chociaż główna jest w AllegroPriceChange.js)
                    parsedData.changes.forEach(change => currentActiveChangesMap.set(String(change.productId), change));
                } else {
        ...
        allPriceBoxes.forEach(priceBox => {
            ...
            if (activeChange) {
                // Ten produkt jest w LS, upewnij się, że przycisk jest aktywny
                if (!priceBox.classList.contains('price-changed')) {
                    priceBox.classList.add('price-changed');
                }
                ...
                if (firstButton && !firstButton.classList.contains('active')) {
                    ...
                    if (actionLine) {
                        // Aktywuj przycisk, pokazując dane z LS
                ...
                // Ten produkt jest oznaczony jako zmieniony, ale nie ma go w LS (np. został usunięty w modale)
                // Zresetuj jego stan
                priceBox.classList.remove('price-changed');
                const item = allPrices.find(p => String(p.productId) === String(productId));
                const priceBoxColumnInfo = priceBox.querySelector('.price-box-column-action');
    ...
    function applyMassChange(changeType) {
        const productsToChange = currentlyFilteredPrices; // Użyj przefiltrowanych widocznych produktów
        ...
        // Pobierz aktualne ustawienia kroku cenowego
        const currentSetStepPrice = setStepPrice;
        const currentUsePriceDifference = usePriceDifference;
        const stepUnit = currentUsePriceDifference ? 'PLN' : '%';

        productsToChange.forEach(item => {
            // Sprawdź, czy zmiana już istnieje w globalnej tablicy (na wszelki wypadek)
            const isAlreadyChanged = selectedPriceChanges.some(c => String(c.productId) === String(item.productId));
            if (isAlreadyChanged) {
                countAdded++; // Licz jako już dodany
                return;
            }

            // Oblicz sugestię na podstawie bieżących ustawień
            const suggestionData = calculateCurrentSuggestion(item);

            // Odrzuć, jeśli nie ma sugestii (np. 'onlyMe', 'noOffer', 'good_no_step')
            ...
            switch (allegroMarginSettings.identifierForSimulation) {
                case 'ID':
                    requiredField = item.myIdAllegro; // Użyj myIdAllegro
                    requiredLabel = "ID";
                    break;
                case 'EAN':
                default:
            ...
            if (allegroMarginSettings.useMarginForSimulation) {
                ...
                if (allegroMarginSettings.enforceMinimalMargin && minMarginPerc >= 0) {
                    if (newMarginPercentage < minMarginPerc) {
                        // Odrzuć, CHYBA ŻE stara marża była jeszcze gorsza, a nowa jest poprawą
                        if (!(oldMarginPercentage < minMarginPerc && newMarginPercentage > oldMarginPercentage)) {
                            countRejected++;
            ...
            // Jeśli wszystko OK, wyślij event (który zostanie przechwycony przez AllegroPriceChange.js)
            const priceChangeEvent = new CustomEvent('priceBoxChange', {
                detail: {
        ...
        // Po pętli, odśwież stan wizualny wszystkich boxów
    ...
    function setupEventListeners() {
        ...
        if (stepPriceInput) {
            stepPriceInput.addEventListener("blur", () => {
                ...
                setStepPrice = parseFloat(stepPriceInput.value.replace(',', '.'));
                updateStepPriceIndicator();
                filterAndSortPrices(); // Przelicz sugestie na żywo
            });
            stepPriceInput.addEventListener('input', updateStepPriceIndicator);
            stepPriceInput.addEventListener('change', updateStepPriceIndicator);
            setTimeout(updateStepPriceIndicator, 0);
                ...
                fetch('/AllegroPriceHistory/SaveAllegroMarginSettings', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            loadPrices(); // Przeładuj dane, aby odświeżyć ustawienia
                        } else {
                            showGlobalNotification('<p style="margin-bottom:8px; font-weight:bold;">Błąd zapisu</p><p>Nie udało się zapisać ustawień Allegro.</p>');
                            hideLoading();
                        }
        ...
        // ### NOWE LISTENERY DLA MASOWEJ ZMIANY ###
    ...
    function loadPrices() {
        ...
        fetch(`/AllegroPriceHistory/GetAllegroPrices?storeId=${storeId}`)
            ...
            .then(data => {
                ...
                // ### DODANE ###
                // Odśwież stan przycisków po załadowaniu i wyrenderowaniu wszystkich cen
                refreshPriceBoxStates();
                // ############
            })
            .catch(error => {
