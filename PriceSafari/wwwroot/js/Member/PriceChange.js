...
//    // --- ULEPSZONA FUNKCJA OBLICZANIA OPŁACALNOŚCI ---
//    function computeOpportunityScore(item, simResult) {
//        if (!simResult) {
//            return {
//                cost: 0,
...
//        const mode = item.mode || 'competitiveness'; // Domyślny tryb

//        // Określamy typ zmiany
//        let basePriceChangeType = 'none';
//        if (priceDiff > 0.005) basePriceChangeType = 'increase';
//        else if (priceDiff < -0.005) basePriceChangeType = 'decrease';
...
//        // Funkcja pomocnicza do pobrania wartości liczbowej rankingu
//        function getRankVal(rData) {
//            if (!rData || rData.rank === null) return 999;
//            return rData.rank;
//        }

//        // --- GŁÓWNY ALGORYTM OCENY (0-100) ---
//        function calculateChannelScore(oldRankData, newRankData, totalOffers, strategyMode) {
//            const oldRank = getRankVal(oldRankData);
//            const newRank = getRankVal(newRankData);

//            if (oldRank === 999 && newRank === 999) return null;

//            let score = 50; // Wartość bazowa
//            const rankChange = oldRank - newRank; // + to poprawa, - to spadek

//            // 1. SCENARIUSZ: LIDER RYNKU (Competitiveness)
//            if (strategyMode === 'competitiveness') {
//                if (basePriceChangeType === 'decrease') {
//                    // Obniżka - celem jest TOP
//                    if (newRank <= 3 && rankChange > 0) score = 95; // Awans do TOP3
//                    else if (rankChange > 0) score = 75; // Awans
//                    else if (rankChange === 0 && newRank <= 1) score = 60; // Utrzymanie lidera (może być OK)
//                    else score = 25; // Przepalanie marży (obniżka bez efektu)
//                }
//                else if (basePriceChangeType === 'increase') {
//                    // Podwyżka - celem jest "Test sufitu"
//                    if (rankChange === 0 && oldRank <= 3) score = 100; // Genialne: drożej, a wciąż w TOP3
//                    else if (rankChange === 0) score = 85; // Drożej, pozycja ta sama
//                    else if (newRank > 3 && oldRank <= 3) score = 30; // Utrata podium - słabo
//                    else score = 45; // Spadek pozycji
//                }
//            }
//            // 2. SCENARIUSZ: RENTOWNOŚĆ (Profit)
//            else if (strategyMode === 'profit') {
//                if (basePriceChangeType === 'increase') {
//                    // Podwyżka - to jest cel tego trybu
//                    if (rankChange === 0) score = 95; // Czysty zysk
//                    else if (newRank <= 10) score = 80; // Spadek, ale wciąż na 1. stronie - OK
//                    else score = 50; // Wylot z 1. strony
//                }
//                else if (basePriceChangeType === 'decrease') {
//                    // Obniżka - tylko jeśli daje duży boost
//                    if (rankChange > 5) score = 70;
//                    else score = 30; // W trybie profit obniżki są niemile widziane
//                }
//            }
...
//        const googleFinalScore = calculateChannelScore(googleOldData, googleNewData, totalG, mode);
//        const ceneoFinalScore = calculateChannelScore(ceneoOldData, ceneoNewData, totalC, mode);

//        // Obliczanie "zysku" pozycji dla starych warunków
//        let calculatedGoogleGained = 0;
//        if (googleOldData.rank && googleNewData.rank) calculatedGoogleGained = googleOldData.rank - googleNewData.rank;

//        let calculatedCeneoGained = 0;
//        if (ceneoOldData.rank && ceneoNewData.rank) calculatedCeneoGained = ceneoOldData.rank - ceneoNewData.rank;

//        return {
//            cost: Math.abs(priceDiff),
//            costFrac: (currentPrice > 0) ? Math.abs(priceDiff) / currentPrice : 0,
//            googleGained: calculatedGoogleGained > 0 ? calculatedGoogleGained : 0,
//            ceneoGained: calculatedCeneoGained > 0 ? calculatedCeneoGained : 0,
//            googleFinalScore,
//            ceneoFinalScore,
...
//                        }
//                    } else if (opp.basePriceChangeType === 'increase') {
//                        // --- NOWA LOGIKA WYŚWIETLANIA DLA PODWYŻEK ---
//                        if (bestScore >= 80) {
//                            // Smart Increase / Zyskowna podwyżka
//                            effectDetails += `<div>
//                                <span style="color: green; font-weight: bold; font-size: 16px;">▲$</span>
//                                <span style="color: green; font-weight: bold; font-size: 16px;"> Zyskowna podwyżka</span>
//                            </div>`;
//                            // Opcjonalnie dodaj wykresy jeśli są wysokie
...
//                        } else {
//                            // Zwykła (bolesna) podwyżka
//                            effectDetails += `<div>
//                                <span style="color: red; font-weight: 400; font-size: 16px;">▲</span>
//                                <span style="color: #222222; font-weight: 400; font-size: 16px;"> Podwyżka ceny</span>
//                            </div>`;
//                            // Pokazujemy wykresy nawet przy złej ocenie (żeby widać było, że słabo)
//                            if (opp.googleFinalScore != null) effectDetails += createDonutChart(opp.googleFinalScore, "/images/GoogleShopping.png");
//                            if (opp.ceneoFinalScore != null) effectDetails += createDonutChart(opp.ceneoFinalScore, "/images/Ceneo.png");
//                        }
//                    } else {
