    ...
    // Pobieramy dane o produktach (w tym EAN) na podstawie ich ID:
    ...
    var modalContent = '<table class="table-orders"><thead><tr>';
        ...
        selectedPriceChanges.forEach(function (item) {
            // UWAGA: sprawdzasz w productDetails TYLKO jeśli potrzebujesz np. imageUrl
            ...
            // Jeżeli wciąż nic nie ma, dopiero wtedy próbujemy z productDetails:
            ...
            // Szukamy dodatkowych informacji z symulacji
            var simResult = simulationResults.find(function (x) { return x.productId == item.productId; });

            // Przygotowujemy wyświetlanie rankingów (Google / Ceneo)
            var googleDisplay = "";
            ...
            modalContent += '<tr>';
                ...
                // *** Wyświetlenie nazwy i EAN pod spodem ***
                modalContent += '<td>' +
                    '<span style="font-size:125%;">' + name + '</span>' +
                    '<br />' +
                    // Tutaj jest wyświetlany EAN
                    '<span>' + ean + '</span>' +
                    '</td>';
