...
//przechodzenie w rozna page plus incognito 
...
//            "--incognito" // Włączenie trybu incognito
//        }
//            });

//            // Otwieranie strony w kontekście incognito
//            var pages = await _browser.PagesAsync();
//            var incognitoPage = pages.FirstOrDefault() ?? await _browser.NewPageAsync();

//            _page = (Page)incognitoPage;
...
//        public async Task HandleCaptchaAndScrapePricesWithNewTabAsync(string url)
//        {
//            try
//            {
//                await _page.GoToAsync(url);
...
//                // Dodajemy zmienną, aby kontrolować, czy CAPTCHA zostało rozwiązane
//                bool captchaSolved = false;

//                // Pętla będzie się powtarzać, dopóki CAPTCHA nie zostanie rozwiązana
//                while (!captchaSolved)
//                {
//                    if (currentUrl.Contains("/Captcha/Add"))
//                    {
//                        Console.WriteLine("CAPTCHA detected. Waiting 5 seconds to open a new tab...");

//                        await Task.Delay(5000);  // Czekaj 5 sekund

//                        var newPage = await _browser.NewPageAsync();  // Otwórz nową kartę

//                        await newPage.GoToAsync(url);  // Przejdź na nową stronę w nowej karcie

//                        Console.WriteLine("Please manually solve the CAPTCHA in the new tab.");

//                        // Czekamy na rozwiązanie CAPTCHA
//                        while (newPage.Url.Contains("/Captcha/Add"))
//                        {
//                            await Task.Delay(10000);  // Sprawdzaj co 10 sekund
//                        }

//                        // CAPTCHA została rozwiązana - przełączamy się na nową kartę
//                        Console.WriteLine("CAPTCHA solved. Resuming scraping...");
//                        _page = (Page)newPage;
//                        captchaSolved = true;  // Ustawienie zmiennej na true oznacza, że możemy wyjść z pętli
//                    }
//                    else
//                    {
//                        Console.WriteLine("No CAPTCHA detected. Continuing with scraping...");
//                        captchaSolved = true;  // Ustawienie zmiennej na true oznacza, że możemy wyjść z pętli
//                    }
...
//                // Po rozwiązaniu CAPTCHA lub jeśli jej nie ma, wykonujemy scraping
//                var (prices, log, rejectedProducts) = await ScrapePricesFromCurrentPage(url, true);
...
//            try
//            {
//                // Użycie nowej metody do obsługi CAPTCHA
//                await HandleCaptchaAndScrapePricesWithNewTabAsync(url);
